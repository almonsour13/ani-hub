  ...
  return (
    <>
      <div className="flex justify-between md:px-4 px-3">
        <h1 className="lg:text-4xl md:text-2xl text-md font-bold">
          {header_title}
        </h1>
        <a className="text-md font-bold">❯</a>
      </div>
      <div
        className="carousel w-full h-auto gap-3 md:scroll-px-4 scroll-px-3 md:px-4 px-3"
        ref={containerRef}
      >
        {data.Page.anime.map((anime: any) => (
          <Link
            ...
            key={anime.idMal}
            className="carousel-item lg:w-1/6 md:w-1/5 sm:w-1/4 w-28 h-auto flex flex-col gap-2"
          >
              <div ref={cardRef} className="w-full lg:h-80 md:h-64 sm:h-60 xs:h-44 h-40 rounded-md bg-accent flex items-center justify-center relative">
                <ImageChecker
              ...
              <div className=" p-0 bg-transparent">
                <div className="line-clamp-2">
                  <p className=" text-xs md:text-base lg:text-xs xl:text-lg font-bold shadow-black">
      ...
      {data.Page.anime.length ? (
        <div className="lg:flex md:flex hidden gap-2 ml-3">
          <button

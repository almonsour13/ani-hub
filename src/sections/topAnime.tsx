          ...
          <div key={anime.id} className="carousel-item w-full relative">
            ...
            >
              <div className="absolute z-10 bottom-0 left-0 w-full">
                <div className="lg:w-11/12 w-full lg:px-0 p-3 flex flex-col gap-1 mx-auto">
                  <h1 className="leading-none lg:text-4xl md:text-2xl text-2xl font-bold text-wrap drop-shadow-md">
                    {anime.title.english || "N/A"}
                  </h1>
                  <p className="leading-none text-xs lg:text-lg md:text-sm">{anime.format}</p>
                  <p className="leading-none text-xs lg:text-lg md:text-sm">{anime.startDate.year} - {anime.endDate.year || "?"}</p>
                  <div className="flex flex-wrap">
                    {anime.genres.map((genre: any,index:number) => (
                      <a href="#" className={`lg:mr-3 lg:pr-3 mr-1 pr-1 leading-none text-xs lg:text-lg md:text-sm ${index !== anime.genres.length-1?'border-r':"border-none"} border-primary`}>
                        {genre}
                      </a>
                    ))}
                  </div>
                  <div className="flex flex-row align-center gap-1 mt-1">
                    <button className="btn lg:btn-md btn-sm w-auto px-4 rounded btn-primary flex flex-row align-center justify-center">
                      <svg
                        className="fill-current lg:h-9 lg:w-9 h-5 w-5"
                      ...
                      <p className="lg:text-xl md:text-lg md:text-sm text-sm">
                        {anime.type === "ANIME"?
                        "Watch Now":"Read Now"}
                      </p>
                    </button>
                    <label className="swap p-0">
                      ...
                      {/* hamburger icon */}
                      <svg
                        className="swap-off fill-current lg:h-12 lg:w-12 h-8 w-8"
                        xmlns="http://www.w3.org/2000/svg"
                        height="24px"
                        viewBox="0 -960 960 960"
                        width="24px"
                      ...
                      {/* close icon */}
                      <svg
                        className="swap-on fill-current lg:h-12 lg:w-12 h-8 w-8"
                        xmlns="http://www.w3.org/2000/svg"
                        height="24px"
                        viewBox="0 -960 960 960"
                        width="24px"
